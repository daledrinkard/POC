...
#if 1 // Micro Controller commands
static int MP_ofs(char** args);
static int MP_dg(char** args);
static int MP_sau(char** args);
static int MP_swap(char** args);
static int MP_f(char** args);
static int MP_rst(char** args);
#endif
const char MP_cmds_name[] = "MP Commands";
const command_t MP_cmds[] =
{
  {"ofs", &MP_ofs,"dump OFS registers"}, //@@@ these commands are not necessarily tied to the control panel
  {"dg",  &MP_dg,"dump memory grid"},    //@@@ this could be a P1 command
  {"sau", &MP_sau,"dump memory grid"},
  {"swap",&MP_swap,"execute a bank swap request"},
  {"f",   &MP_f,"flash utility"},
  {"stat",&MP_stat,"status flag"},
  {"rst", &MP_rst,"restart app"},
  {"creg",&MP_creg,"creg reg value"},
  {"tmr",&MP_tmr,"tmr Timer [T|P] [value]"},  //@@@ needs flushing out
  //{"adc",&MP_adc,"adc command stub"},
  {0,0,0}
};
...
#if 1 // CP commands
/*
 *   MP_dg   Dump a grid of the specified memory
 */
...
static void exMP_stat_identify(void);
static int MP_stat(char** args)  /* stat s value | c value | r */
{
    uint32_t v;
    ...
    else /* write*/
    {
        if (NULL == args[2]) /* these options have no argument */
        {
            switch (args[1][0]) {
            case 'i':
            case 'I': exMP_stat_identify(); return 0;
            }
            return -1;
        }
