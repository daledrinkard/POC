...
typedef enum e_hmsc_error_codes {
    HMSC_ERROR_NONE = 0,
    HMSC_ERROR_UX_SYSTEM_FAILED = 1,
    HMSC_ERROR_UX_HOST_STACK_FAILED = 2,
    HMSC_ERROR_USB_OPEN_FAILED = 3
}hmsc_error_codes_t;
typedef struct s_hmsc {
    hmsc_states_t state;
    hmsc_error_codes_t error;
    hfile_t    hfile[1]; //@@@ number of files
	FX_MEDIA * p_media;
	time_format_t g_set_time;
	uint8_t    ux_pool_memory[MEMPOOL_SIZE];
}hmsc_t;
