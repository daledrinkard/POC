...
int app_func_reset   (void)
{
    ...
    fsp_err_t err;
    HMSC.state = HMSC_STATE_DISMOUNT;
    HMSC.error = HMSC_ERROR_NONE;
    fx_system_initialize();
    ux_return = ux_system_initialize((CHAR *)HMSC.ux_pool_memory, MEMPOOL_SIZE, UX_NULL, RESET_VALUE);
    if (UX_SUCCESS != ux_return)
    {
        HMSC.error = HMSC_ERROR_UX_SYSTEM_FAILED;
        return -1;
    }
    ux_return =  ux_host_stack_initialize(apl_change_function);
    if (UX_SUCCESS != ux_return)
    {
        HMSC.error = HMSC_ERROR_UX_HOST_STACK_FAILED;
        return -1;
    }
        APP_INFO_PRINT("ux host stack initialized successfully!");

        /* Open usb driver */
        err = R_USB_Open(&g_basic0_ctrl, &g_basic0_cfg);
        if (FSP_SUCCESS != err)
        {
            HMSC.error = HMSC_ERROR_USB_OPEN_FAILED;
            return -1;
        }

        APP_INFO_PRINT("Init completed");
                                   ...
                                   _ux_utility_string_length_get(_ux_system_host_class_storage_name)))
    {
        switch (event)
        {
            ...
            case UX_FSP_DEVICE_INSERTION:
            {
                ...
                HMSC.p_media     = &storage_media->ux_host_class_storage_media;
                tx_event_flags_set(&App.events, SYSFLG_USB_PLUG_IN, TX_OR);
                break;
            }

            /* Check if there is a device removal. */
            case UX_FSP_DEVICE_REMOVAL:
            {
                HMSC.p_media = UX_NULL;
                tx_event_flags_set(&App.events, SYSFLG_USB_PLUG_OUT, TX_OR);
                break;
            }
...
static void time_stamping(void)
{
    /* Extract current Date and Time from compiler MACROS */
        ...
        {
            HMSC.g_set_time.date -= DATE_VALUE_SUB;
        }

        /* Convert returned month name to month value */
        assign_month_value(&HMSC.g_set_time, &read_time[0]);

        /* Set system date and time */
        fx_system_date_set(HMSC.g_set_time.year, HMSC.g_set_time.month, HMSC.g_set_time.date);
        fx_system_time_set(HMSC.g_set_time.hour, HMSC.g_set_time.min, HMSC.g_set_time.sec);
}
